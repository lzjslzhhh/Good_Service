...
export const getCurrentUser = () => {
  return request.get('/admin/me').then((res: any) => res.data as User)
}

// 3. 更新当前用户信息
export const updateCurrentUser = (payload: Record<string, any>) => {
  return request.put('/admin/me', payload).then((res: any) => res.data as void)
}

// 4. 管理员修改指定用户
export const updateUser = (id: number, payload: Record<string, any>) => {
  return request.put(`/admin/users/${id}`, payload).then((res: any) => res.data as void)
}

// 5. 获取统计数据 (按月趋势)
export const getMonthlyStats = (params: { start?: string; end?: string; region?: string }) => {
  return request
    .get('/admin/stats/monthly', { params })
